...
import com.tencao.saoui.util.J8String;
import com.tencao.saoui.screens.window.WindowAlign;
import com.tencao.saoui.screens.window.SubWindow;
...
import com.tencao.saoui.social.friends.FriendList;
import com.tencao.saoui.social.party.PartyHelper;
import com.tencao.saoui.social.party.PartyHandler;
...
public class IngameMenuGUI extends ScreenGUI {
    ...
    private void openMenu(Elements element, Categories id) {
        ...
        if (id == Categories.PROFILE) {
            ...
            menu.elements.add(new ButtonGUI(menu, Categories.SKILLS, 0, 0, StatCollector.translateToLocal("guiSkills"), IconCore.SKILLS));
            ...
            menu.elements.add(new ButtonGUI(menu, Categories.FRIENDS, 0, 0, StatCollector.translateToLocal("guiFriends"), IconCore.FRIEND));
            ...
            if (OptionCore.DEBUG_MODE.getValue()) menu.elements.add(new ButtonGUI(menu, Categories.DUNGEON_MAP, 0, 0, StatCollector.translateToLocal("guiDungMap"), IconCore.DUNGEON_MAP));
            ...
            menu.elements.add(new ButtonState(menu, Categories.LOGOUT, 0, 0, OptionCore.LOGOUT.getValue() ? StatCollector.translateToLocal("guiLogout") : "", IconCore.LOGOUT, (mc1, button) -> {
                if (OptionCore.LOGOUT.getValue()) {
        ...
        //Profile
            ...
            menu.elements.add(new ButtonGUI(menu, Categories.ARMOR, 0, 0, StatCollector.translateToLocal("guiEquipped"), IconCore.ARMOR));
        ...
        //Profile -> Equipment
            ...
            if (OptionCore.COMPACT_INVENTORY.getValue()){
                ...
                if (mc.thePlayer.inventoryContainer.getInventory().stream().filter(st -> st != null).anyMatch(st -> InventoryCore.AXE.isFine((ItemStack) st, true))) menu.elements.add(new ButtonGUI(menu, Categories.AXE, 0, 0, StatCollector.translateToLocal("guiAxes"), IconCore.EQUIPMENT));
                if (mc.thePlayer.inventoryContainer.getInventory().stream().filter(st -> st != null).anyMatch(st -> InventoryCore.SHOVEL.isFine((ItemStack) st, true))) menu.elements.add(new ButtonGUI(menu, Categories.SHOVEL, 0, 0, StatCollector.translateToLocal("guiShovels"), IconCore.EQUIPMENT));
        ...
        //Social
            ...
            setInfo(null, null);
            menu = new FriendList(mc, element, menuOffsetX, menuOffsetY, 100, 100);
        ...
        //Navigation
            ...
            sub = SubWindow.resetCheckPositionSub(mc, sub, mc.thePlayer, 4, '-' + StatCollector.translateToLocal("guiFieldMap") + '-');
            info = SubWindow.addInfo(sub);
            ...
            sub = SubWindow.resetCheckPositionSub(mc, sub, mc.thePlayer, 1, '-' + StatCollector.translateToLocal("guiDungMap") + '-');
            info = SubWindow.addInfo(sub);
